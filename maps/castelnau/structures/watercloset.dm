...
/obj/structure/hygiene/shower/attack_hand(var/mob/M)
	...
	if(on)
		QDEL_NULL(sound_token)
		playsound(src.loc, 'sound/ark/machines/shower_start.ogg', 40)
		sound_token = GLOB.sound_player.PlayLoopingSound(src, sound_id, 'sound/ark/machines/shower_mid3.ogg', volume = 20, range = 7, falloff = 4, prefer_mute = TRUE)
		if (M.loc == loc)
			wash(M)
			process_heat(M)
		...
		QDEL_NULL(sound_token)
		playsound(src.loc, 'sound/ark/machines/shower_end.ogg', 40)
