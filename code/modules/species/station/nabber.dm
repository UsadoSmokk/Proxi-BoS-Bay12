...
/datum/species/nabber/handle_environment_special(mob/living/carbon/human/H)
	if(!H.on_fire && H.fire_stacks < 2 && H.species.get_bodytype() != SPECIES_MONARCH_QUEEN) //boh
		H.fire_stacks += 0.2
	return
...
/datum/species/nabber/get_additional_examine_text(var/mob/living/carbon/human/H)
	if(H.species.get_bodytype() == SPECIES_MONARCH_QUEEN) //boh
		return ..()
...
/datum/species/nabber/attempt_grab(var/mob/living/carbon/human/grabber, var/mob/living/target)
	if(grabber.species.get_bodytype() == SPECIES_MONARCH_QUEEN) //boh
		return ..()
...
/datum/species/nabber/monarch_queen //boh
...
/datum/species/nabber/monarch_worker //boh
