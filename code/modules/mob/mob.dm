...
/mob/proc/is_mechanical()
	if(mind && (mind.assigned_role == "Robot" || mind.assigned_role == "AI"))
		return 1
	return istype(src, /mob/living/silicon) || get_species() == SPECIES_IPC || get_species() == SPECIES_SHELL
...
/mob/proc/UpdateLyingBuckledAndVerbStatus()
	...
	if(lying)
		set_density(0)
		if(l_hand) if(l_hand.w_class > ITEM_SIZE_NORMAL) unEquip(l_hand) // BoS
		if(r_hand) if(r_hand.w_class > ITEM_SIZE_NORMAL) unEquip(r_hand) // BoS
	else
