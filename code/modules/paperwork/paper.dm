...
/obj/item/paper
	...
	var/datum/language/language = LANGUAGE_HUMAN_LORD // Language the paper was written in. Editable by users up until something's actually written
...
/obj/item/paper/New(loc, text, title, list/md = null, datum/language/L = null)
	...
	if (!set_language(language, TRUE))
		...
		set_language(LANGUAGE_HUMAN_LORD, TRUE)
