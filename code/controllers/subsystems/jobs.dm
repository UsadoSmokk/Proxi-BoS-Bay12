...
/datum/controller/subsystem/jobs/proc/equip_rank(var/mob/living/carbon/human/H, var/rank, var/joined_late = 0)
	...
	if(job.supervisors)
		to_chat(H, "<b>Должность вашего начальника: [job.supervisors]. Однако во время игры это может измениться.</b>")

	to_chat(H, "<b>Осмотрев наушник, вы узнаете доступные вам радиоканалы. Канал вашего отдела доступен по префиксу \":h\"</b>")

	if(job.req_admin_notify)
		to_chat(H, "<b>Вы играете на важной игровой роли. Если вам нужно отключиться - предупредите администрацию.</b>")
	...
	INVOKE_ASYNC(GLOBAL_PROC, .proc/show_location_blurb, H.client, 30) //nebula
...
/proc/show_location_blurb(client/C, duration) //nebula
	set waitfor = 0

	var/style = "font-family: 'Fixedsys'; -dm-text-outline: 1 black; font-size: 11px;"
//	var/area/A = get_area(C.mob)
	var/text = "[stationdate2text()], [stationtime2text()]\nShip: [station_name()]\nLocation: Cadaab - moon of the Inax,\nthe Mjolnir system"
	text = uppertext(text)
